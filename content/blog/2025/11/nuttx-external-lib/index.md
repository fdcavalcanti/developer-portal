...
This article has three parts: building a sample library on x86, adding it to the NuttX simulation environment, and finally, cross-compiling to RISC-V and running it on the ESP32-C6.

## Using an example library

As an example, use an application that converts a hexadecimal color string to RGB. The reference project is available in [this repository](https://github.com/fdcavalcanti/hex-converter) and has the following structure:
...
The `hex-converter` library exposes one single function which is called `hex_to_rgb`. The user provides a pointer to a string represeting the hex color and a pointer
to an array where the R, G and B values will be copied into. It is a simple application but very useful as an example.
...
As an user, I want to use that library in my application. The first solution might be to copy the entire hex-converter repository to the NuttX application directory and 
...
2. On `apps/hello/Make.defs`, add the hex library, library path and include path.
...
Success! We are able to compile our library externally, link it to a NuttX application and use it.

## Using the library on ESP32C6

Now that simulation works, we must look into a real use case that would require the same code to work on hardware.
...
With a simple change of compiler and no changes on the NuttX build system, we were able to have the same example
running on a ESP32-C6.
